...
Minimale Serverlast, schnelle UI, lokale Datenhaltung im Speicher, Push-Updates über Broker.
...
- Nach Änderungen: `git add` + `git commit`. **Kein `git push` durch Codex/Kiro**, Nutzer pusht selbst.
...
Wichtige Commands:
- `cmd=bootstrap` liefert Nutzer, Menü, Rooms, Config, etc. (nur einmal beim Login)
- `cmd=state` liefert Versions-Hash (leichtgewichtiger Poll-Check)
- `cmd=pricelevel_state` liefert Preisstufen-Version
- `cmd=refresh_tables` liefert aktuelle Tischdaten (bei Broker-Push oder Poll-Fallback)
- `cmd=refresh_menu` liefert aktuelles Menü/Preise (bei Preisstufen-Änderung)
- `cmd=order`, `cmd=paydesk_items`, `cmd=paydesk_pay` usw.
...
- Modern UI arbeitet mit aktuellem Preis aus `bootstrap` (im Speicher gehalten).
- Broker überwacht Preisstufenänderung und pusht `scope: MENU`.
- Bei Preisstufenänderung: Modern UI lädt nur Menü/Preise neu via `cmd=refresh_menu` (nicht volles bootstrap).
...
Daten-Strategie (ab v22):
- `bootstrap` wird einmal beim Login geladen. Menü, Tische, Config bleiben im Speicher.
- Tischdaten werden nur bei Broker-Push (`scope: TABLES`) oder Poll-Fallback (State-Hash-Änderung) vom Server geholt.
- Menü/Preise werden nur bei Broker-Push (`scope: MENU`) neu geladen, via `cmd=refresh_menu`.
