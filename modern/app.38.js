...
function openProductModal(prod) {
  ...
  const noteFieldHtml = comments.length > 0 ? `
    <div class="note-input-group">
      ...
      <input type="text" id="modal-note-input" class="note-input" placeholder="oder manuell eingeben..." />
    </div>
  ` : `
    <input type="text" id="modal-note-input" class="note-input" placeholder="Bemerkung..." />
  `;
  
  // Replace the note field content
  if (els.modalNote) {
    els.modalNote.innerHTML = noteFieldHtml;
    
    // Handle predefined comment selection
    if (comments.length > 0) {
      const select = els.modalNote.querySelector("#modal-note-select");
      const input = els.modalNote.querySelector("#modal-note-input");
      if (select && input) {
        select.onchange = () => {
          if (select.value) {
...
function addProductToCart() {
  ...
  // Get note from modal - check for the new input field
  let note = "";
  const noteInput = els.productModal.querySelector("#modal-note-input");
  if (noteInput) {
