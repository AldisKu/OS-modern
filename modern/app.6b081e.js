...
function renderPriceKeyboard() {
  if (!els.priceKeypad) return;
  const rows = [
    ["1", "2", "3"],
    ["4", "5", "6"],
    ["7", "8", "9"],
  ...
  rows.forEach(row => {
    ...
    row.forEach(label => {
      ...
      if (label === "Back") btn.classList.add("key-wide");
      btn.onclick = (e) => handlePriceKey(label, e);
      btn.ontouchstart = (e) => handlePriceKey(label, e);
      rowEl.appendChild(btn);
    });
    els.priceKeypad.appendChild(rowEl);
  });
}
