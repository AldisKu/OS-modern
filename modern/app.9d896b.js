...
function renderOrderItems() {
  ...
  existingGroups.forEach(g => {
    ...
    if (Number(g.item.unit || 0) === 0 && basePrice > 0 && itemPrice < basePrice - 0.0001) {
...
function renderPaydeskItems() {
  ...
  els.paydeskOpen.innerHTML = openGroups.map(g => {
    ...
    const unitVal = Number(g.item.unit || (state.menu?.prods || []).find(p => Number(p.id) === Number(g.item.prodid))?.unit || 0);
    if (unitVal === 0) {
