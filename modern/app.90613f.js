...
function bindModals() {
  ...
  if (els.priceConfirm) {
    els.priceConfirm.addEventListener("click", () => {
      ...
      if (!state.priceEntry.digits) {
        alert("Bitte Preis eingeben.");
        return;
      }
      const value = formatPriceFromDigits(state.priceEntry.digits, state.priceEntry.negative);
      const price = value.toFixed(2);
      const togo = state.selectedTable?.id === 0 ? 1 : 0;
      addToCartCustom(state.priceEntry.prod, [], "", 1, togo, price);
      els.priceModal.classList.add("hidden");
      state.priceEntry = null;
    });
  }
  if (els.priceKeypad) {
    els.priceKeypad.addEventListener("click", (e) => {
      const btn = e.target.closest("button");
      if (!btn || !state.priceEntry) return;
      const key = btn.dataset.key;
      if (!key) return;
      if (key === "back") {
        state.priceEntry.digits = state.priceEntry.digits.slice(0, -1);
      } else if (key === "neg") {
        state.priceEntry.negative = !state.priceEntry.negative;
      } else if (key === "comma") {
        // no-op: decimal input is implied by cents
      } else if (/^\\d$/.test(key)) {
        if (state.priceEntry.digits.length < 9) {
          state.priceEntry.digits += key;
        }
      }
...
function formatPriceFromDigits(digits, negative) {
...
function updatePriceModalDisplay() {
  ...
  const value = formatPriceFromDigits(state.priceEntry.digits, state.priceEntry.negative);
  els.priceValue.textContent = value.toFixed(2);
}

function openPriceModal(prod) {
  if (!prod) return;
  state.priceEntry = { prod, digits: "", negative: false };
