...
function openProductModal(prod) {
  ...
  // Build note field with predefined comments dropdown + free text
  ...
  const noteFieldHtml = comments.length > 0 ? `
    <div class="note-input-group">
      ...
      <input type="text" id="modal-note" class="note-input" placeholder="oder manuell eingeben..." />
    </div>
  ` : `
    <input type="text" id="modal-note" class="note-input" placeholder="Bemerkung..." />
  `;
  
  // Handle predefined comment selection
  if (comments.length > 0) {
    // We'll set up the event listener after the modal is rendered
    setTimeout(() => {
      const select = els.productModal.querySelector("#modal-note-select");
      const input = els.productModal.querySelector("#modal-note");
      if (select && input) {
        select.onchange = () => {
          if (select.value) {
            input.value = select.value;
            select.value = ""; // Reset select for next use
          }
        };
      }
    }, 0);
  ...
  if (els.modalNote) {
    els.modalNote.value = "";
    // Replace the note field with our new HTML
    const noteContainer = els.modalNote.parentElement;
    if (noteContainer) {
      noteContainer.innerHTML = noteFieldHtml;
    }
  }
...
function addProductToCart() {
  ...
  // Get note from modal - check both the new select+input combo and the old input field
  let note = "";
  const noteInput = els.productModal.querySelector("#modal-note");
  if (noteInput) {
    note = noteInput.value || "";
  }
  
  addToCart(prod, sanitizeExtras(extras), note, qty);
  els.productModal.classList.add("hidden");
  renderOrderItems();
}
...
function openLocalConfigModal() {
  ...
  const commentsList = comments.map((c, idx) => `
    <div class="comment-item">
      ...
      <button type="button" class="ghost comment-delete-btn" data-idx="${idx}">×</button>
    </div>
  `).join("");
  ...
  els.confirmBody.innerHTML = `
    ...
    <div class="config-section">
      <h3>Bemerkungen verwalten</h3>
      <div class="comments-list" id="comments-list">
        ${commentsList || "<p style='color: #999;'>Keine Bemerkungen definiert</p>"}
      </div>
      <div class="comment-input-row">
        <input type="text" id="new-comment-input" placeholder="Neue Bemerkung..." maxlength="50" />
        <button type="button" class="ghost" id="add-comment-btn">Hinzufügen</button>
      </div>
    </div>
  `;
  ...
  // Add comment button
  els.confirmBody.querySelector("#add-comment-btn").onclick = () => {
    const input = els.confirmBody.querySelector("#new-comment-input");
    const text = (input.value || "").trim();
    if (!text) {
      alert("Bitte Bemerkung eingeben");
      return;
    }
    if (!state.localConfig.comments) state.localConfig.comments = [];
    if (state.localConfig.comments.includes(text)) {
      alert("Diese Bemerkung existiert bereits");
      return;
    }
    state.localConfig.comments.push(text);
    saveLocalConfig();
    openLocalConfigModal(); // Refresh modal
  };
  
  // Delete comment buttons
  els.confirmBody.querySelectorAll(".comment-delete-btn").forEach(btn => {
    btn.onclick = () => {
      const idx = Number(btn.dataset.idx);
      if (state.localConfig.comments && idx >= 0 && idx < state.localConfig.comments.length) {
        state.localConfig.comments.splice(idx, 1);
        saveLocalConfig();
        openLocalConfigModal(); // Refresh modal
      }
    };
...
function loadLocalConfig() {
  try {
    ...
    if (!raw) return { singleExtraImmediate: true, tableLayout: null, comments: [] };
    const parsed = JSON.parse(raw);
    return {
    ...
    return { singleExtraImmediate: true, tableLayout: null, comments: [] };
  }
}

function saveLocalConfig() {
  try {
    localStorage.setItem("modern_local_config", JSON.stringify(state.localConfig || { singleExtraImmediate: true, tableLayout: null, comments: [] }));
  } catch (_) {}
}
